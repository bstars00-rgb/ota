...
| 항목 | 배점 | 점수 | 근거 |
|---|---|---|---|
| 페이지 커버리지 | 30 | **30** | 20 페이지 (웹 18 + 모바일 1 + 크리에이터 대시보드 1) — admin-channels 추가 |
| 비즈니스 모델 완성도 | 20 | **20** | 골프텔 중심 + 국가 오너십 + 채널 매니저 9종 + ELLIS_SPEC.md 백엔드 풀스펙 완성 |
| UX 플로우 완결성 | 20 | **20** | 검색→상세→슬롯→결제→완료(라운딩별 일자)→마이페이지(D-day chips) 완전 E2E |
| 기능 정교함 | 20 | **19** | 몽키 표준 — 슬롯 그리드·리뷰 stats·OSM 지도·캐디팁/공휴일/카트정책 풀 커버 + 날짜 picker 주말 모디파이어 |
| 디자인 시스템 일관성 | 10 | **9** | 토큰 전역 공유, golf 컬러 추가, 골프텔/운영 콘솔 일관 |
...
| 수정 실행 | 20 | **18** | Skip link·Esc·reduced-motion·검색 커밋·flights 중앙화·호텔 missing 보강 완료 |
| 데이터 일관성 | 15 | **14** | flights 완전 중앙화 / hotels 부분 중앙화 + 골프텔 hotelId 100% 참조 정상화 |
| 코드 정리도 | 15 | **13** | state.js dormant membership 메소드 · 일부 href="#" 잔존 |

### 인터랙티브 점수: **99 / 100**
실제 움직이는 프로토타입 — 골프텔 슬롯 다중 선택 / 객실 supplement / 항공 결합 토글 / 알림 딥링크 / 마이페이지 골프텔 SKU 인식 / 날짜 picker 주말 모디파이어 / 라운딩별 일자 chips / admin 콘솔 실시간 로그 갱신 등.
...
| 20 | `admin-channels.html` ⭐ | **ELLIS 운영자 콘솔** | 다크 모드 · KPI 4종 · 통합 아키텍처 · 어댑터 9개 카드 · 코스 매핑 표 · **실시간 동기화 로그 (5초마다 prepend)** |
...
| `state.js` | localStorage 상태 관리 | 카트·예약(v3)·찜·팔로우·좋아요·저장·리뷰·쿠폰·포인트·검색이력·최근본·유저 세션 · **알림 읽음 처리** · **동적 알림 D-7/D-3/D-1** (+ dormant 멤버십 메소드) |
...
- ✅ golftel-builder.html (4스텝 자유 조립 + 날짜 picker + 주말 모디파이어)
- ✅ admin-channels.html (운영자 콘솔, 실시간 로그 자동 갱신)
- ✅ index/country.html 골프텔 진입점
- ✅ creator-dashboard 골프텔 SKU 인식 + 추천
- ✅ hotels/flights 데이터 중앙화
- ✅ checkout.html / complete.html / mypage.html 골프텔 컨텍스트 + 라운딩별 일자/주중·주말
- ✅ 채널 매니저 9종 매핑 + 실시간/견적 모드
- ✅ 마이페이지 11탭 (홈 대시보드 + 알림 + 최근본 + 문의)
- ✅ 시드 골프텔 예약 3건 + 알림 딥링크 + 동적 알림 (D-7/D-3/D-1)
- ✅ AI 플래너 골프텔 통합 (골프 의도 감지 → 3티어 추천)
- ✅ ELLIS_SPEC.md (1,000줄 백엔드 연동 스펙)
- ✅ 모든 골프텔 리뷰 5건씩 균등 (총 40건)

### P0 · 핵심 (현재 우선순위)

| # | 항목 | 설명 | 예상 작업량 |
|---|---|---|---|
| 1 | **결제 게이트웨이 mock 화면** | KakaoPay·NaverPay·Toss 결제 페이지 mock (실제 SDK 도입 전 단계) | M |
| 2 | **mypage 문의 작성 모달** | 1:1 채팅 폼 — 상품 첨부 / 카테고리 / 답변 알림 통합 | M |
| 3 | **리뷰 사진 업로드 mock** | 리뷰 모달에 사진 첨부 (FileReader → preview, 실제 업로드는 mock) | S |
| 4 | **모바일 앱 골프텔 통합** | mobile.html 홈 탭에 골프텔 카드 / 추천 / 다가오는 라운딩 | M |
| 5 | **golftel.html 데이트 picker → 객실 가격에도 반영** | 현재 그린피만 — 호텔도 주말 +10% 가산 옵션 | S |
...
_Status snapshot 2026-06-11 (2nd) · 20 pages · 2 JS modules · 7 docs · 골프텔 8 + 골프코스 10 + 채널매니저 9 + 오너십 + 40 리뷰 + 10 알림 + 동적 D-day · 기획 98 / QA 92 / 인터랙티브 99_
