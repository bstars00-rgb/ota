...
| 항목 | 배점 | 점수 | 근거 |
|---|---|---|---|
| 페이지 커버리지 | 30 | **30** | **24 페이지** (웹 22 + 모바일 1 + 크리에이터 1) — mapper / voucher / admin-suppliers / payment 추가 |
...
| # | 항목 | 설명 | 예상 작업량 |
|---|---|---|---|
| 1 | **검색 → 상품 상세 사용자 흐름 분석 도구** | A/B 추적 mock (운영자용 GA 대시보드) | L |
| 2 | **국가별 인바운드 광고 랜딩** | 베트남/일본 → "한국 골프 여행" 인바운드 SEO 페이지 | L |
| 3 | **크리에이터 라이브 commerce 모듈** | 쇼츠 + 실시간 시청 + 즉시 구매 (몽키 차별점) | XL |
| 4 | **e-바우처 → 카카오톡 알림톡 API 연동 (실제)** | mock → 실제 카카오톡 알림톡 BizPlus 연동 | L |
| 5 | **다국어 (영/일)** | 인바운드 대응 — 추후 결정 | L |
...
_Status snapshot 2026-06-11 (5th) · **24 pages** · 2 JS modules · 7 docs · **PWA 설치 가능** · 골프텔 8 + 골프코스 10 + 채널매니저 9 + 공급사 9 + 오너십 + 40 리뷰 + 10 알림 + 동적 D-day + 사용자 알림 + 자동 답변 + 결제 게이트웨이 3종 + e-바우처 + 공급사 인벤토리 + **검색 자동완성 + FCM 푸시 mock + 카톡 알림톡** · 기획 **100** 🎯 / QA **96** / 인터랙티브 **100**_
