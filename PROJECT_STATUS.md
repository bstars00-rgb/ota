# 오마이트립 프로토타입 — 프로젝트 상태
**마지막 업데이트**: 2026-07-10 (계정 이관 후속 — 최신 델타 반영 · 정본은 `git log`)
**리포지토리**: https://github.com/bstars00-rgb/ota
**라이브**: https://bstars00-rgb.github.io/ota/
**백엔드(예정)**: ELLIS — 프론트 데이터 모델은 ELLIS 마이그레이션 호환으로 설계

> ⚠️ 이 문서의 점수·상세 표는 **2026-06-12 스냅샷** 기준입니다. 그 이후 변경분은 아래 "최근 변경 이력"과 `git log`가 정본이며, 본문 세부 수치(예: 페이지 수 36)는 아직 갱신 전입니다.

---

## 🆕 최근 변경 이력 (2026-06-12 이후 · git log 기준)

계정 이관(OPS → CEO Office, 2026-07-08) 전후로 반영된 주요 작업:

| 커밋 | 내용 |
|---|---|
| `ba679e6` | 골프텔 티오프 슬롯 시간대 아코디언 — 스크롤 91% 감소(25,953→2,417px) |
| `9bf031c` | PWA SW 캐시 버전 업(`omt-v3-2026-07-09`) |
| `aac4d32` | **신규 페이지** `video-to-trip.html` — 콘텐츠→골프텔 변환 데모 |
| `01c3d60` | 크리에이터 매직 링크 어트리뷰션(링크→예약 전환 추적) |
| `9eb2229` | AI 플래너 추천 카드에 찜·장바구니·티오프 정보 추가 |
| `580fead` | **이관 문서** `handover.md` 추가 (계정 이관 정본) |
| `81b4244` | 골프텔 티오프 라운드별 재설계(같은 시간 재사용 + 코스 필터) |
| `0dd7dc5` | 골프텔 상세 헤더 리뷰 개수 `[object Object]` 수정 |
| `cff6b2a` | 호텔 '인기 도시' 그리드 삭제 |
| `2cda425` | 호텔 목록 미표시(TDZ) 버그 수정 + 트러스트 지표 제거 |
| `7d966f3` | 골프+항공 실시간 항공편 선택(고정 추정치 → 실제 편 선택) |

**현재 실측 규모**: HTML **40개**(문서 헤더의 "36"은 이후 페이지 추가로 변동 — `video-to-trip.html` 등), JS 4개(`data.js`/`state.js`/`icons.js`/`sw.js`), MD 문서 12개.

**이관 핵심 규칙**은 [handover.md](handover.md) §14 참조(메인 SKU 골프텔 / 국가 오너십 / 백엔드 MD-only / 다크모드 정책 / 에어텔 폐지 / 마일리지 단일 리워드 / 채널 매니저).

**알려진 미정리 항목**: `checkout.html`이 마일리지와 별개로 `addPoints`(포인트)를 적립 — 마일리지 단일 리워드 정책과 중복. 포인트는 `mypage.html` 탭·리뷰 보너스·`admin-customers.html`에 연동돼 있어 통합 시 리팩터 필요(결정 대기).
...
| 항목 | 배점 | 점수 | 근거 |
|---|---|---|---|
| 페이지 커버리지 | 30 | **30** | **36 페이지** (에어텔 패키지 3종 폐지: trip-builder·packages·product) · 골프텔(핵심)+항공+호텔 단품 중심 + 백오피스 8 |
| 비즈니스 모델 완성도 | 20 | **20** | + **회원 등급 5단 자동 산정** + 마일리지(단일 리워드)·할부 시스템 + 다이내믹 패키지 + 가격 알리미 |
| UX 플로우 완결성 | 20 | **20** | + 항공 단독 결제 풀 분기 + 호텔 단독 상세 8탭 + 호텔 4개 비교 + 재방문 인사 + 다이내믹 패키지 빌더 |
| 기능 정교함 | 20 | **20** | + **시트립 22종 베스트 프랙티스** (항공 5 + 호텔 5 + 상세·비교 + 추가 12) + 가격 추이 그래프 + 라이트박스 + 지도 보기 + 7일 캘린더 popover + 빠른 예약 |
| 디자인 시스템 일관성 | 10 | **10** | 다크 모드 정책 메모리 유지 · 백오피스 다크 + 사용자 라이트 + 시트립 색감 가미 |

### QA 점수: **100 / 100** 🎯

| 항목 | 배점 | 점수 | 근거 |
|---|---|---|---|
| 체크리스트 완전성 | 20 | **20** | QA_CHECKLIST.md v4.0 — 21 섹션 (v3 13 + §15~§21 신규 7 섹션: 시트립 항공 5 / 호텔 5 / 호텔 상세·비교 / 회원 시스템 / 검색 결과 / 기타 / 사용자 피드백) |
| 기준의 구체성 | 15 | **15** | ELLIS_API_BACKOFFICE.md v1.0 + BACKOFFICE_GUIDE.md v1.0 + BACKOFFICE_SPEC.md v1.2 + QA_CHECKLIST v4.0 |
| 갭 식별 정확도 | 15 | **15** | QA_CHECKLIST.md §14 미구현 작업 + 사용자 피드백 14건 §21 모두 명시 |
| 수정 실행 | 20 | **20** | 결제 실패·D-day·환불 4-eye·정산 승인·그룹 할인·항공 단독·AI 검색·hero 줄바꿈·checkout 안정성 모두 처리 |
| 데이터 일관성 | 15 | **15** | state.js KEYS 일관 (`omt_*_v1`) + visitCount/miles(단일 리워드)/priceAlerts 일관 + groups DDL + ELLIS 호환 |
| 코드 정리도 | 15 | **15** | 헬퍼 패턴 통일 (`get/set/add/remove/use/earn/increment`) + CSS 토큰 일관 + try-catch 안전 처리 + 다크모드 정책 메모리 |

### 인터랙티브 점수: **100 / 100**
실제 움직이는 프로토타입 — 골프텔 슬롯 / 객실 supplement / 알림 딥링크 / 결제 3종 + 10% 실패 / 문의 자동답변 / 리뷰 사진 / AI 음성 5초 인식 + Q&A 15 키워드 / 비교 모드 / 온보딩 진행률 / FAB / 동선 D-1 카드 / 그룹 5스텝 + N분의1 / 알림 카테고리 토글 + 방해 금지 / **항공 30일 가격 추이 + 가격 알리미 + 7일 가격 비교 + 취소 보장 / 호텔 카드 hover 7일 캘린더 popover + 지도 보기 토글 + 4개 비교 모달 / 호텔 풀스크린 Lightbox + 비슷한 호텔 / 회원 등급 자동 5등급 + 마일리지(단일 리워드)·할부 자동 + 빠른 예약 / 재방문 사용자 자동 인사**.
...
_Status snapshot 2026-06-12 (28.5th · 만점 복구 🎯) · 39 pages · 2 JS modules · 11 docs (**QA_CHECKLIST v4.0** +7 섹션) · 시트립 22종 + 호텔 상세·비교 + 회원 시스템 강화 + 사용자 피드백 14건 · 152 tasks · 기획 **100** 🎯 / QA **100** 🎯 / 인터랙티브 **100** 🎯_
...
| **28 (#151~)** | **🔧 사용자 피드백 — checkout 안정성 강화** | tierBenefitBlock 초기 display:none → 보임 (브론즈 기본) + setTimeout(100ms) → 즉시 실행 + try-catch 안전 처리 + coins.balance fallback 12,500P + slider.max 최소 500 보장 + expressBtn 항상 노출 |
| **28.5** | **🎯 만점 회복 (기획 + QA + 인터랙티브 = 100/100/100)** | **QA_CHECKLIST.md v4.0** — §15 시트립 항공 5종 / §16 시트립 호텔 5종 / §17 호텔 상세 + 비교 모드 / §18 회원 시스템 (등급·마일·코인·할부·빠른예약) / §19 항공·호텔 검색 결과 페이지 / §20 기타 시트립 추가 (지도·재방문·패키지·Lightbox) / §21 사용자 피드백 즉시 대응 14건 풀 커버 · QA 96 → 100 회복 (체크리스트 +3 / 갭 식별 +1) · PROJECT_STATUS 점수 카드 사이클 28 시점 재산정 |
| **36** | **🪙 리워드 단일화 — 트립코인 폐지 → 마일리지 통합** | 사용자 지적("코인 이름이 트립코인?" = 경쟁사 시트립 'Trip Coins' 차용 + 마일리지와 역할 중복). **state.js**: `tripCoins` KEY·`getTripCoins/useTripCoins/earnTripCoins` 제거 → `miles` 단일 리워드로 통합 (기본 잔액 12,500마일 이관 + `useMiles/earnMiles` 신규, 1마일=1원). **checkout.html**: '추가 할인' 블록의 마일리지 자유입력(100마일=1,000원) + 트립코인 슬라이더 2종 → **마일리지 사용 슬라이더 1종**(보유 잔액 기반)으로 통합. computeTotal/INIT 동기화. trip-builder.html "마일리지 적립" 문구는 항공사 FFP(스카이팀·대한항공)라 유지. |
| **37** | **✈️ 야놀자 항공 4종 패턴 적용** | 사용자 야놀자 항공 플로우 캡처 기반 고도화. **flights.html**: ① 날짜 선택 캘린더에 **날짜별 mock 최저가 표시**(만원 축약 "43.5만"·결정론적 sine, 월별 최저가 초록·주말 빨강·셀 디자인 원형 day+가격 2단) + 하단 CTA가 선택 기간 동적 표시("6.24(수)~6.26(금) 선택"). **flights-results.html**: ② **요금 선택 모달**(야놀자 패턴 — KB·삼성 최저가 외 13개 카드사별 가격, 라디오 선택 → 선택완료 → checkout) ③ **운임규정 모달**(규정1 수수료: 변경 KRW 40,000 + 취소/환불 시점별 표 / 규정2 수하물·마일리지·여행일정순위·일반규정) ④ **빈 상태 쿠폰·번들 강화**(항공권 구매 시 최대 15% 할인 + 골프텔 결합 CTA) ⑤ **즉시할인 프로모 배너**(노선 최저가 기준 3~10만원 인당 즉시할인). **checkout.html**: 항공 단독 결제에 `card`·`fare` 파라미터 반영(카드별 요금 + 적용 카드 표시). |
| **38** | **🗑️ 에어텔(자유여행 패키지) 라인 폐지 — FIT 트렌드 전환** | 사용자: "패키지는 삭제, 에어텔인데 요즘 트렌드에 맞지 않음". **페이지 삭제**: `trip-builder.html`·`packages.html`·`product.html` 3종. **데이터**: `data.js` PRODUCTS 12건 → `[]`. **진입 정리**: index(GNB/드로어 패키지→항공·호텔 교체, 다이내믹 패키지 빌더 CTA 제거, 오리지널 섹션을 **직영 골프텔**로 재활용, footer/검색인덱스/quick-tab 정리), search(패키지 빌더 CTA→3 SKU 바로가기, 📦 패키지 필터·인덱스 제거), mobile(패키지 메뉴·홈 단독특가 섹션·검색인덱스·상품카드 제거), country(오리지널·빌더 섹션 제거, golf-card→golftels), cart(추천상품 블록 제거), mypage(위시·리뷰·최근본 product→hotel/golftel 분기), checkout(패키지 링크→골프텔). **방향**: 골프텔(핵심) + 항공·호텔 단품. 9개 파일 인라인 스크립트 errors 0, 삭제 페이지 잔여 링크 0건 검증. |
