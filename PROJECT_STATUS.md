...
| 항목 | 배점 | 점수 | 근거 |
|---|---|---|---|
| 페이지 커버리지 | 30 | **30** | **27 페이지** (웹 25 + 모바일 1 + 크리에이터 1) — analytics / inbound / live 추가 |
...
| 27 | `live.html` ⭐ | **크리에이터 라이브 commerce** | LIVE 펄스 + 시청자 카운터 + 채팅 시뮬 + 재고 -1 (6초 간격) + 좋아요/팔로우/공유 + 박동 구매 버튼 → payment 직진 |
...
| # | 항목 | 설명 | 예상 작업량 |
|---|---|---|---|
| 1 | **search.html 직영 우선 정렬 추가** | ⭐ OMT 직영 → 일반 골프텔 → 기타 타입 + 평점순 보조 | S ✅ |
| 2 | **모바일 LIVE 시작 알림 시뮬** | 진입 45초 후 데모 푸시 + 권한 granted 시 50초 후 | S ✅ |
| 3 | **AI 플래너 ↔ search 혼합** | 추천 3 카드 다음에 "직접 매칭 전체 보기" 카드 추가 | S ✅ |
| 4 | **크리에이터 대시보드 LIVE 통계** | 누적 시청자/평균 시청시간/LIVE 전환율/이번주 LIVE 회수 | M ✅ |
| 5 | **(다음 사이클)** 검색 → 상품 후 자동 추천 / 라이브 다운로드 클립 등 | 후속 디테일 보강 | M |
...
_Status snapshot 2026-06-11 (7th) · **27 pages** · 2 JS modules · 8 docs · PWA 설치 가능 · 골프텔 8 + 골프코스 10 + 채널매니저 9 + 공급사 9 + 오너십 + 40 리뷰 + 10 알림 + 동적 D-day + 사용자 알림 + 자동 답변 + 결제 게이트웨이 3종 + e-바우처 + 공급사 인벤토리 + 검색 자동완성 + FCM 푸시 mock + 카톡 알림톡 + 사용자 흐름 분석 + 인바운드 SEO 3개 언어 + 라이브 commerce + **search 직영 우선 정렬 + LIVE 시작 알림 시뮬 + AI 혼합 모드 + 크리에이터 LIVE 통계** · 기획 **100** 🎯 / QA **96** / 인터랙티브 **100**_
