...
| 항목 | 배점 | 점수 | 근거 |
|---|---|---|---|
| 페이지 커버리지 | 30 | **30** | 19 페이지 (웹 17 + 모바일 1 + 크리에이터 대시보드 1) — golftels/golftel/golftel-builder 추가 |
| 비즈니스 모델 완성도 | 20 | **20** | 골프텔 중심 + 국가 오너십 + 채널 매니저 9종 매핑 + ELLIS 호환 정규화 완성 |
| UX 플로우 완결성 | 20 | **19** | 검색→상세(8탭)→슬롯 선택→결제→완료(골프텔 컨텍스트)→마이페이지(홈 대시보드) E2E 작동 |
| 기능 정교함 | 20 | **19** | 몽키 표준 — 슬롯 그리드(170개)·리뷰 stats·OSM 지도·캐디팁/공휴일/카트정책 풀 커버 |
| 디자인 시스템 일관성 | 10 | **9** | 토큰 전역 공유, golf 컬러 추가, 골프텔 페이지 일관 |
...
| 기준의 구체성 | 15 | **14** | WCAG 실측값, API 스키마 명세 |
| 갭 식별 정확도 | 15 | **14** | ⚠️ 마커로 명시 |
| 수정 실행 | 20 | **18** | Skip link·Esc·reduced-motion·검색 커밋·flights 중앙화 완료 |
| 데이터 일관성 | 15 | **14** | flights 완전 중앙화 / hotels 부분 중앙화(LOCAL+병합) — ELLIS 호환 정규화 |
| 코드 정리도 | 15 | **12** | state.js dormant membership 메소드 · 일부 href="#" 잔존 |

### 인터랙티브 점수: **98 / 100**
실제 움직이는 프로토타입 — 골프텔 슬롯 다중 선택 / 객실 supplement / 항공 결합 토글 / 알림 딥링크 / 마이페이지 골프텔 SKU 인식 등.
...
| 17 | `golftels.html` ⭐ | **골프텔 검색·리스트** | 8개 골프텔 · 카트/시설 필터 · 직영/파트너 뱃지 · 리뷰 stats 카드 · 빌더 진입점 |
| 18 | `golftel.html` ⭐ | **골프텔 상세 (8탭)** | 갤러리 8장 + lightbox / 1박=1라운드 매칭 배너 / 객실 옵션 selector / 슬롯 그리드 (8분 간격, 시단위 그룹, 코스 뱃지, 실시간 ⚡ / 견적 24h) / 이용안내 · 위치(OSM iframe) · 리뷰 27건 · FAQ 8건 |
| 19 | `golftel-builder.html` ⭐ | **골프텔 빌더** | 4스텝 — 국가/도시 → 호텔 → 코스(다중) → 옵션·항공 · 실시간 가격 + sticky 요약 |

### 공용 스크립트 (2)

| 파일 | 역할 | 내용 |
|---|---|---|
| `data.js` | 중앙 목데이터 | 4 국가 · 23 공항 · 13 항공사 · 34 항공편 · 25 호텔 · 25 액티비티 · 12 상품 · **8 골프텔 (full content)** · **10 골프코스 (yardage)** · **9 채널 매니저 (CHANNEL_MANAGERS)** · **국가 오너십 매트릭스** · **PRODUCT_TYPES enum** · 8 크리에이터 · 8 피드 · 8 리뷰 · **6 시드 예약 (골프텔 3 + 패키지 3)** · 5 쿠폰 · 6 포인트 · **4 크리에이터 등급** · **10 알림 + 4 문의 mock** · **27 골프텔 리뷰 mock** |
| `state.js` | localStorage 상태 관리 | 카트·예약(v3)·찜·팔로우·좋아요·저장·리뷰·쿠폰·포인트·검색이력·최근본·유저 세션 · **알림 읽음 처리** (+ dormant 멤버십 메소드) |
...
### ✅ 완료된 P0 (골프텔 중심 전환 — 2026-05~06)
- ✅ golftels.html (검색·리스트, 카트/시설 필터, 직영/파트너 뱃지)
- ✅ golftel.html (8탭 상세 + 슬롯 그리드 + OSM 지도 + 27건 리뷰)
- ✅ golftel-builder.html (4스텝 자유 조립)
- ✅ index/country.html 골프텔 진입점
- ✅ creator-dashboard 골프텔 SKU 인식 + 추천
- ✅ hotels/flights 데이터 중앙화
- ✅ checkout.html / complete.html / mypage.html 골프텔 컨텍스트
- ✅ 채널 매니저 9종 매핑 + 실시간/견적 모드
- ✅ 마이페이지 11탭 (홈 대시보드 + 알림 + 최근본 + 문의)
- ✅ 시드 골프텔 예약 3건 (D-3 다낭/D-28 클락/완료 오키나와)

### P0 · 핵심 (현재 우선순위)

| # | 항목 | 설명 | 예상 작업량 |
|---|---|---|---|
| 1 | **채널 매니저 어댑터 대시보드** (admin-channels.html) | 운영자용 — GDO/GolfNow/Golfdigg/Direct 어댑터 연결 상태 / 동기화 시각 / 인벤토리 / 실시간율 / 골프장 매핑 시각화 | M |
| 2 | **AI 플래너 골프텔 통합** | "골프" 키워드 감지 → GOLFTELS에서 3티어 추천 (가성비/밸런스/프리미엄) | S |
| 3 | **동적 알림 생성** | 정적 NOTIFICATIONS + 사용자 예약 dates 기반 D-7/D-3/D-1 자동 생성 | S |
| 4 | **ELLIS 백엔드 연동 스펙 문서** | data.js → ELLIS API 매핑 (필드·엔드포인트·스키마 차이) | M |
| 5 | **complete.html 시드 골프텔 데모 보강** | OMT-3DK21A/CLK55B/OKI09C 직접 접속 시나리오 검증 | S |
...
_Status snapshot 2026-06-11 · 19 pages · 2 JS modules · 6 docs · 골프텔 8 + 골프코스 10 + 채널매니저 9 + 오너십 + 27 리뷰 + 10 알림 · 기획 97 / QA 90 / 인터랙티브 98_
