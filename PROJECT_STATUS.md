...
| 항목 | 배점 | 점수 | 근거 |
|---|---|---|---|
| 페이지 커버리지 | 30 | **30** | 21 페이지 (웹 19 + 모바일 1 + 크리에이터 대시보드 1) — payment.html 추가, 전 결제 흐름 완결 |
| 비즈니스 모델 완성도 | 20 | **20** | 골프텔 중심 + 국가 오너십 + 채널 매니저 9종 + ELLIS_SPEC.md 백엔드 풀스펙 완성 |
| UX 플로우 완결성 | 20 | **20** | 검색→상세→슬롯→checkout→**payment→complete(자동 알림)**→마이페이지 완전 E2E + 문의 자동 답변 + 결제 실패 재시도 |
| 기능 정교함 | 20 | **20** | 몽키 표준 + 결제 게이트웨이 3종 + 호텔 주말 모디파이어 + 사진 업로드 + 사용자 알림 시스템 |
| 디자인 시스템 일관성 | 10 | **9** | 토큰 전역 공유, golf 컬러 추가, payment 게이트웨이 브랜드별 컬러 |
...
| 수정 실행 | 20 | **19** | 결제 실패 시나리오·재시도·자동 알림 트리거 추가로 엣지 케이스 대응 |
...
| # | 항목 | 설명 | 예상 작업량 |
|---|---|---|---|
| 1 | **complete.html → 알림 D-day 트리거 자동화** | 결제 완료 시점에 D-7/D-3 알림을 미리 예약 (현재는 클라이언트 진입 시 동적 생성) | M |
| 2 | **모바일 알림 탭 골프텔 통합** | mobile.html 알림 탭이 현재는 정적 — 동적 알림 + 사용자 알림 반영 | M |
| 3 | **장바구니에 골프텔 담기** | golftel.html → 장바구니 (현재는 즉시 결제만) | M |
| 4 | **공급사 어드민 페이지 (admin-suppliers.html)** | 액티비티/렌트 공급사 등록·재고관리 (admin-channels.html 자매 페이지) | L |
| 5 | **e-바우처 PDF 미리보기** | complete.html에 "바우처 다운로드" → 새 창에 골프장 입장권/호텔 체크인 정보 PDF 시각화 | M |
...
_Status snapshot 2026-06-11 (3rd) · **21 pages** · 2 JS modules · 7 docs · 골프텔 8 + 골프코스 10 + 채널매니저 9 + 오너십 + 40 리뷰 + 10 알림 + 동적 D-day + **사용자 알림 + 자동 답변 + 결제 게이트웨이 3종** · 기획 **99** / QA **93** / 인터랙티브 **100**_
