...
| 항목 | 배점 | 점수 | 근거 |
|---|---|---|---|
| 페이지 커버리지 | 30 | **30** | 23 페이지 (웹 21 + 모바일 1 + 크리에이터 1) — voucher / admin-suppliers 추가 |
| 비즈니스 모델 완성도 | 20 | **20** | 골프텔 중심 + 채널 매니저 9 + 공급사 9 + ELLIS_SPEC.md 백엔드 풀스펙 완성 |
| UX 플로우 완결성 | 20 | **20** | 검색→상세→슬롯→checkout→payment→complete(자동 알림+e-바우처)→마이페이지 완전 E2E |
| 기능 정교함 | 20 | **20** | 몽키 표준 + 결제 3종 + 모디파이어 + 사진 업로드 + 사용자 알림 + e-바우처 인쇄 + 공급사 인벤토리 |
...
| # | 항목 | 설명 | 예상 작업량 |
|---|---|---|---|
| 1 | **마이페이지 통합 검색** | 예약번호/상품명/리뷰/문의 한 번에 검색 (탭 자동 전환) | M |
| 2 | **golftels.html 날짜 필터** | 상단 날짜 picker → 카드별 예상 가산률 미리보기 | M |
| 3 | **데이터 모델 매핑 도구 (mapper.html)** | data.js → ELLIS DDL 자동 생성 (운영자용 백엔드 마이그레이션 가이드) | M |
| 4 | **모바일 결제 게이트웨이 통합** | mobile.html → payment.html 폰 프레임 내 결제 흐름 | M |
| 5 | **AI 플래너 다국어 통합** (P1로 강등 가능) | 영어/일본어 자연어 처리 (인바운드 외국인 대응) | L |
...
_Status snapshot 2026-06-11 (4th) · **23 pages** · 2 JS modules · 7 docs · 골프텔 8 + 골프코스 10 + 채널매니저 9 + **공급사 9** + 오너십 + 40 리뷰 + 10 알림 + 동적 D-day + 사용자 알림 + 자동 답변 + 결제 게이트웨이 3종 + **e-바우처 + 공급사 인벤토리** · 기획 **100** 🎯 / QA **94** / 인터랙티브 **100**_
