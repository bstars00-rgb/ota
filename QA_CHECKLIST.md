...
### 15.1 트러스트 시그널 4 카드 (`flights.html`)
- [ ] 페이지 진입 시 4 카드 표시 — 17 항공사 / 총액 표시 / 최저가 보장 / 24/7 카톡
- [ ] 카드 아이콘 배경색 (주황·블루·그린·옐로우)
- [ ] 1024px 이하 모바일 — grid 1~2열로 재배치

### 15.2 30일 가격 추이 그래프
- [ ] `renderPriceTrend()` 진입 시 15일 바차트 렌더
- [ ] 주말 빨간색 / 평일 주황 / 최저가 초록 강조
- [ ] 클릭 시 자동 출발일 설정 + 검색 폼 스크롤
- [ ] 최저가/평균/피크 가격 카드 3종 표시

### 15.3 가격 알리미 모달
- [ ] "🔔 가격 알리미 설정" 버튼 클릭 시 모달 표시
- [ ] 노선 + 현재 최저가 + 평균 표시
- [ ] 목표가 입력 (10만 이상) + 빠른 선택 4종 (30/35/40/50만)
- [ ] 출발 월 6 옵션 + 알림 채널 3종 (카톡/이메일/푸시)
- [ ] state.addPriceAlert 저장 + DevTools localStorage 확인

### 15.4 항공+골프텔 번들 CTA
- [ ] 그라데이션 배너 표시 — "최대 25% 더 저렴"
- [ ] 골프텔-air 진입 + 단체 견적 진입

### 15.5 flights-results.html 7일 가격 추이
- [ ] 상단 7일 가격 미니 그래프 표시
- [ ] 현재 선택일 그라데이션 강조 + 최저가 초록
- [ ] 클릭 시 dep 파라미터 변경 reroute
- [ ] 각 카드에 ✓ 총액 표시 + 🛡️ 취소 보장 +5% 옵션 (체크 시 토스트)

---

## 16. 시트립 호텔 5종 (사이클 21)

### 16.1 트러스트 시그널 5 카드 (`hotels.html`)
- [ ] 페이지 상단 5 카드 — 1,624,549 숙소 / 164M+ 리뷰 / ⭐4.7 / 💰 최저가 보장 / 📞 24/7
- [ ] 카드 사이 border-left 구분선

### 16.2 인기 호텔 도시 그리드 7 권역
- [ ] 7 권역 탭 표시 (국내·일본·중국·아시아·미주·유럽·대양주)
- [ ] 전체 탭 클릭 — 각 권역 2개씩 추출 (대표 14 도시)
- [ ] 권역 탭 클릭 — 해당 권역만 표시
- [ ] 도시 카드 — 이모지·도시명·코드·호텔 수·1박 최저가·추이 (↗↘)
- [ ] 호버 시 translateY + 그림자

### 16.3 호텔+골프텔 번들 CTA
- [ ] 다크 그린 풀 width 배너
- [ ] "라운딩 추가 시 평균 18% 더 저렴" 카피
- [ ] 골프텔 8선 + AI 추천 진입
...
- [ ] ✓ 무료 취소 (해당 호텔만)
- [ ] ⭐ 멤버 -7% (해당 호텔만)
- [ ] 🔥 지금 N명이 보는 중 (일부 호텔 펄스 애니메이션)

### 16.5 권역 탭 동적 그리드
- [ ] selectRegion() 호출 시 전환 정상
- [ ] 호버 효과 + 펄스 애니메이션 CSS 정상

---

## 17. 호텔 상세 + 비교 모드 (사이클 26)

### 17.1 hotel.html 8 탭 구조
- [ ] 5분할 헤더 갤러리 (메인 2x + 4 + "+N장 더보기")
- [ ] sticky 탭 바 7개 (개요/객실/시설/리뷰/위치/정책/FAQ)
- [ ] 스크롤 시 탭 자동 활성화
- [ ] 우측 sticky 예약 카드 (가격/날짜/CTA/4 트러스트)

### 17.2 객실 4종 비교 카드
- [ ] 스탠다드/슈페리어(조식+)/디럭스(전망)/스위트 4 종 표시
- [ ] 각 객실: 크기·침대·뷰·조식 포함·무료 취소·잔여실·1박 가격
- [ ] "선택 →" 클릭 시 sessionStorage 저장 + cart 진입

### 17.3 리뷰·평점 분포 + 위치
- [ ] 평점 6 카테고리 막대 차트 (청결/시설/위치/직원/편안함/가성비)
- [ ] 5건 mock 리뷰 카드
- [ ] OpenStreetMap iframe + 역/공항/도심 3 카드

### 17.4 정책 + FAQ
- [ ] 정책 4 카드 (취소·환불 / 어린이 / 결제 / 반려동물)
- [ ] FAQ 5개 (조식/늦은 체크인/픽업/한국어/와이파이) 토글

### 17.5 hotels.html 비교 모드
- [ ] 호텔 카드 좌측 상단 비교 체크박스
- [ ] 체크 시 하단 sticky 드로어 등장 (N/4)
- [ ] 5번째 추가 시 alert
- [ ] 2개 미만일 때 비교 모달 비활성
- [ ] 비교 모달 10 행 표 (호텔/평점/위치/등급/가격/시설/취소/즉시확정/멤버/상세)
- [ ] selectHotel 라우팅 → hotel.html?id=...

---

## 18. 회원 시스템 강화 — 등급·마일·코인·할부·빠른예약 (사이클 27)
...
- [ ] coins.balance가 undefined일 때 12,500P 기본
- [ ] slider.max 최소 500 보장
- [ ] 즉시 실행 (setTimeout 제거)

---

## 19. 항공·호텔 검색 결과 페이지 (사이클 19 + 26)

### 19.1 flights.html — 인터파크 스타일
- [ ] 왕복/편도/다구간 트립 타입 탭
- [ ] 직항 항공 토글
- [ ] 6 grid 검색 폼 (출발지·⇄·도착지·일정·인원·검색)
- [ ] 도시 그리드 모달 (9 권역 검색)
- [ ] 2개월 캘린더 모달
- [ ] 인원·좌석등급 모달 (4 좌석 × 성인/아동/유아)
- [ ] 인기 노선 8 카드 + 빠른 진입 4 카드

### 19.2 flights-results.html
- [ ] 상단 stepper (가는편→오는편→결제)
- [ ] 좌측 필터 4 섹션 (경유/항공사/가격대/시간)
- [ ] 정렬 chips 4종 (각 가격·시간 미리보기)
- [ ] 항공편 카드 (그라데이션 로고/3-col 노선/잔여석)
- [ ] 빈 상태 UI

### 19.3 checkout.html 항공 단독 분기 (사이클 22)
- [ ] URL params (type=flight/id/adult/child/infant/cls/dep/ret) 추출
- [ ] flight-only pkg 구성
- [ ] renderFlightOnlySummary 표시 (가는편/돌아오는편)
- [ ] computeTotal: flight.price × paxCount

---

## 20. 기타 시트립 추가 패턴 (사이클 27)

### 20.1 hotels.html 추가 4종
- [ ] 보기 모드 토글 (목록↔지도) + OpenStreetMap 임베드
- [ ] Urgency 시그널 (847건 예약 / 2h 가격 변동)
- [ ] 호텔 카드 hover 시 7일 가격 캘린더 popover
- [ ] 주말 빨강·최저가 초록 + 최저가 +N일 후 안내

### 20.2 hotel.html 추가 3종
- [ ] Urgency 배너 (마지막 12분 / 28명 보는 중 / 24h 142건)
- [ ] 갤러리 풀스크린 Lightbox (좌우 ◀▶ + 썸네일 reel + 카운터)
- [ ] 비슷한 호텔 4개 자동 추천 (같은 도시 우선)

### 20.3 index.html 재방문 인사
- [ ] 방문 2회차+ 시 인사 카드 표시
- [ ] "방문 N회차 + 지난번 본 상품 4개 그리드"
- [ ] × 버튼으로 닫기 가능

### 20.4 search.html 다이내믹 패키지
- [ ] 다크 그린 풀폭 배너 표시
- [ ] "항공+호텔 -15%" 카피 + 골프 추가 -5% 안내
- [ ] 3 채널 진입 (항공/호텔/골프텔)

---

## 21. 사용자 피드백 즉시 대응 (사이클 22~25, 28)

### 21.1 항공 단독 결제 (사이클 22)
- [ ] flights-results "선택" → checkout?type=flight 정상 진입
- [ ] "예약할 상품이 없어요" 빈 상태로 안 빠짐

### 21.2 메인 검색창 (사이클 23~24)
- [ ] placeholder = "여행지·호텔·항공·골프텔 검색" (AI 문구 없음)
- [ ] 아이콘 = 🔍 (✦ 아님)
- [ ] aria-label = "통합 검색"
- [ ] 빈 입력 + 키워드 모두 search.html로

### 21.3 hero 텍스트 (사이클 25)
- [ ] "행." 한 글자 떨어짐 없음
...
**체크리스트 버전**: v4.0 · 2026-06-12 (사이클 28 시점)
**커버리지**: 39 페이지 · 시트립 22종 + 호텔 상세 + 비교 + 회원 시스템 강화 + 사용자 피드백 14건
